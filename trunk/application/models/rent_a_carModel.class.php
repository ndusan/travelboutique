...
public function getPageInfo($params, $langId){
		...
		$query = sprintf("SELECT * FROM `pages` WHERE `link`='rent-a-car'");
...
public function getBanners($pageInfo, $langId){
	...
	public function setRequest($params){
		...
		$sql = "INSERT INTO `rent_a_car` SET ";
		$num = 0;
		foreach($params['rent_a_car'] as $key => $val)
			$sql .= "`".$key."`='".mysql_real_escape_string($val)."'".(++$num<count($params['rent_a_car'])?", ":" ");
		mysql_query($sql);
